<?php
	...
	$annot = $r->getAllTracksByProject($page,$start,$rp,$sortname,$sortorder,$wh,$uid,$pid);
...
?>
	...
	if ($annot) {
		...
		foreach ( $annot as $a ) {
			echo '<row id = "'.$a->iddas_commonserver_das_tracks.'">';
			echo '<cell><![CDATA['.$a->iddas_commonserver_das_tracks.']]></cell>';
			echo '<cell><![CDATA['.$a->dname.']]></cell>';
			echo '<cell><![CDATA['.$a->url.']]></cell>';
