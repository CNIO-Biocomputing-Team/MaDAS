...
* Version: .1
*/
var map,layers,popup,bp,chr;

function init(das_server,pid,current_dsn,segment,size){ 

    var server = 'plugins_dir/visualization/madasmap/wms.php?das_server='+das_server+'&current_project='+pid+'&current_dsn='+current_dsn+'&segment='+segment+'&size='+size;
...
function getFeature(tx,ty,sstart){
  ...
  if (ty >= 200){
      ...
      $jQ('#bubble').load('plugins_dir/visualization/madasmap/features.php?r=1&start='+sstart+'&stop='+sstop+'&h='+h,function(data){
      ...
      $jQ('#bubbler').load('plugins_dir/visualization/madasmap/features.php?r=0&start='+sstart+'&stop='+sstop+'&h='+h,function(){
      	...
      	if (type){
	        ...
	        $jQ("#bubbler").hide();
      		...
      		if (e.pageY > 400){
		    	...
		    	$jQ('#bubble').load('plugins_dir/visualization/madasmap/getPockets.php?r=1&start='+start+'&stop='+stop+'&type='+type);
		    	...
		    	$jQ('#bubbler').load('plugins_dir/visualization/madasmap/getPockets.php?r=0&start='+start+'&stop='+stop+'&type='+type);
	    	}
